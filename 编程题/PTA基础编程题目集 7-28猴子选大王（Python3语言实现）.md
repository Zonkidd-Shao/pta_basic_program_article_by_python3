...
在0下标编号下令最后幸存者为0；从2只猴子递推到N只，每次用`(result + 3) % i`更新幸存位置，最后加1还原为题目编号。

## 复杂度分析

程序利用约瑟夫问题递推式，从 2 迭代到 N 一次，时间复杂度为 O(N)，额外空间复杂度为 O(1)。
