# 7-4 BCD解密（Python3语言实现）

## 前言

本题（7-4 BCD解密）的主要考点是：准确理解题意、规范处理输入输出格式，并正确处理边界与精度。下面先给出题目描述与格式要求，再通过清晰的思路与可直接运行的python3代码进行讲解。
...
result = high_digit * 10 + low_digit
...
1. **读取输入**：使用`input()`读取一个正整数，通过`int()`转换后存入变量`n`。
2. **提取高位数字**：`n // 16`整除16，得到十六进制表示的高位数字（对应BCD的十位）。
3. **提取低位数字**：`n % 16`取模16，得到十六进制表示的低位数字（对应BCD的个位）。
4. **组合正确结果**：高位数字×10 + 低位数字，得到正确的十进制数。
5. **输出结果**：使用`print()`输出正确的十进制数。
...
把错误的十进制数转换成十六进制数位：`// 16`取得高位，`% 16`取得低位，再按十进制的十位和个位重新组合。

## 复杂度分析

程序只处理固定的两位 BCD 数字，包含常数次进制转换操作，时间复杂度为 O(1)，额外空间复杂度为 O(1)。
