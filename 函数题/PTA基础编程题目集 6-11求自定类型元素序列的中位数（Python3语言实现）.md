...
对N个元素进行原地排序，时间复杂度为`O(N log N)`；排序所需的额外空间取决于Python排序实现，通常可记为`O(N)`，函数返回一个元素。

### 常见易错点

1. 题目定义的中位数是第⌊(N+1)/2⌋大的元素，偶数个元素时应取排序后下标`N // 2`的位置，而不是两个中间值的平均数。
2. 必须先排序再取元素，不能假设输入已经有序。
3. 函数修改了数组A的顺序；若调用方需要保留原顺序，应先复制，但题目接口允许原地排序。
4. 应返回ElementType元素本身，不要在函数内部格式化输出。

### 总结

`Median`先将序列排序，再按题目给定的排名规则取出中位数，核心是正确处理偶数长度时的下标定义。
