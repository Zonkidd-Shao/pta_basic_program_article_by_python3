...
求和需要遍历N个元素，时间复杂度为`O(N)`，只保存总和与平均值，额外空间复杂度为`O(1)`。

### 常见易错点

1. 平均值应为总和除以N，不能除以数组实际长度以外的数量。
2. 需要返回浮点结果，不能使用整除。
3. 题目保证N为正数时无需额外处理除零；若扩展到空数组，应先约定返回值。
4. 输出格式由裁判程序控制，函数只需返回平均值。

### 总结

`Average`先累加前N个元素，再除以元素个数得到平均值，函数本身只负责计算并返回结果。
