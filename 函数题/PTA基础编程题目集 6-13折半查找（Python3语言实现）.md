...
每次比较都会将候选区间缩小约一半，因此时间复杂度为`O(log N)`；函数只使用区间端点和中点变量，额外空间复杂度为`O(1)`。

### 常见易错点

1. 记录表`R`按题目约定从下标1开始，不能直接按Python列表的0下标规则初始化查找区间。
2. 左半区间更新为`end = mid - 1`，右半区间更新为`start = mid + 1`，否则可能死循环。
3. 命中时应返回位序`mid`，查找失败时返回0，由裁判程序转换为NOT FOUND。
4. 二分查找要求数列严格递增；无序数据不能直接套用该算法。

### 总结

`Search_Bin`利用有序性反复折半缩小查找区间，命中返回1起始的位序，区间耗尽则返回0。
