...
函数需要输出N个整数，因此时间复杂度为`O(N)`；除循环变量外不需要额外存储，空间复杂度为`O(1)`。

### 常见易错点

1. `range(1, N)`不会包含N，应使用`range(1, N + 1)`。
2. 每个数字都应单独占一行，不能在同一行用空格连接。
3. 题目要求提交函数时只实现`PrintN`，不要重复定义裁判程序中的输入和调用部分。

### 总结

`PrintN`通过一次顺序循环完成1到N的逐行输出，关键是正确处理`range`的右端点和换行格式。
