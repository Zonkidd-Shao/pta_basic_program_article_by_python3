...
多项式共有N+1个系数，函数循环N次，因此时间复杂度为`O(n)`；只维护当前幂和累加值，额外空间复杂度为`O(1)`。

### 常见易错点

1. 常数项应从`a[0]`开始，不能从`a[1]`开始累加。
2. 每轮应先把`m`乘以`x`，再使用`a[i] * m`，否则幂次会错位。
3. 系数数组的下标范围是0到n，循环应包含最高次项。
4. 不必对每一项重复调用幂运算，递推维护当前次幂即可。

### 总结

函数从常数项出发递推计算各次幂，并将对应系数项累加，在线性时间内完成多项式求值。
